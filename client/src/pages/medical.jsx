...
export default function App() {
    ...
    const loadStores = useCallback(async (latitude, longitude) => {
        ...
        try {
            ...
            if (!data) throw new Error("Servers unreachable");
                    ...
                    <div className="max-w-7xl mx-auto">
                        {loading ? (
                            <div className="h-[60vh] flex flex-col items-center justify-center text-center">
                                ...
                                <h3 className="font-bold text-slate-800">Searching Nearby Facilities...</h3>
                            </div>
                        ) : error ? (
                            <div className="h-[60vh] flex flex-col items-center justify-center text-center">
                                <AlertTriangle className="text-red-500 mb-4" size={48} />
                                <p className="text-slate-600 font-medium">{error}</p>
                                <button onClick={() => window.location.reload()} className="mt-4 px-6 py-2 bg-slate-900 text-white rounded-xl text-xs font-bold uppercase">Retry</button>
                            </div>
                        ) : (
                            <>
                                <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                    <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Results for your area</h2>
                                ...
                                {displayList.length === 0 && (
                                    <div className="py-20 text-center">
                                        <p className="text-slate-400 font-bold text-sm uppercase">No units found in this range</p>
                                    </div>
                                )}
                            </>
                        )}
                    ...
                    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-4">
                        ...
                        <div className="relative bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-10 sm:zoom-in-95 duration-300">
                            <div className="h-48 bg-slate-100 relative">
                                ...
                                />
                                <button onClick={() => setSelectedHospital(null)} className="absolute top-4 right-4 bg-white p-2 rounded-full shadow-lg hover:text-red-500">
                                    <X size={20} />
                                </button>
                            </div>
                            <div className="p-6 sm:p-8">
                                <div className="flex gap-2 mb-4">
                                    ...
                                    <span className="bg-slate-100 text-slate-600 text-[9px] font-black px-2 py-1 rounded-md uppercase">{selectedHospital.distance.toFixed(2)} KM</span>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-3 mb-4">
                                    <a href={`tel:${selectedHospital.phone}`} className={`flex items-center justify-center gap-2 p-3 rounded-xl border text-[10px] font-black uppercase ${selectedHospital.phone ? "border-slate-200 hover:bg-emerald-50" : "opacity-30 pointer-events-none"}`}>
                                        <Phone size={14} /> Call
                                    </a>
                                    <button onClick={() => {navigator.clipboard.writeText(selectedHospital.address); alert("Copied!");}} className="flex items-center justify-center gap-2 p-3 rounded-xl border border-slate-200 text-[10px] font-black uppercase hover:bg-blue-50">
                                        <Copy size={14} /> Copy
                                    </button>
                                </div>
                                <a
                                    ...
                                    className="flex items-center justify-center gap-2 w-full bg-slate-900 text-white py-4 rounded-xl text-xs font-bold uppercase hover:bg-emerald-600 transition-all"
                                >
